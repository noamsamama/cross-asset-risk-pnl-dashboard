...
export default function App() {
  ...
  useEffect(() => {
    if (window.location.pathname === '/') window.history.replaceState(null, '', '/P&L')
  ...
  return (
    <Box sx={{ display: 'flex', minHeight: '100vh', bgcolor: 'grey.50' }}>
      <Box
        component="aside"
        sx={{ width: 220, flexShrink: 0, borderRight: 1, borderColor: 'divider', bgcolor: 'white' }}
      >
        <Typography variant="h6" sx={{ p: 2.5 }}>
          RAD Risk Tool
        </Typography>
        <List>
          <ListItemButton selected={view === 'pnl'} onClick={() => navigate('pnl')}>
            <ListItemText primary="Positions & P&L" />
          </ListItemButton>
          <ListItemButton selected={view === 'risk'} onClick={() => navigate('risk')}>
